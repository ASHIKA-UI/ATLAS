...
class StudyGraph:
    ...
    def _load_static_files(self):
        ...
        cuts_path = self._find_data_path(
            "cuts.csv"
        )
        ...
        ref_path = self._find_data_path(
            "reference_ranges.csv"
        )

        if ref_path is not None:
            self.reference_ranges = pd.read_csv(
                ref_path
            )

        corrections_path = self._find_data_path(
            "corrections.csv"
        )

        if corrections_path is not None:
            self.corrections = pd.read_csv(
                corrections_path
            )
    ...
    def _protocol_for_cut(self, cut):
        ...
        latest = available.sort_values(
            "cut"
        ).iloc[-1]

        if "protocol_version" in latest.index:
    ...
    def _apply_corrections(
        ...
        # Apply corrections from earliest
        # to latest so the latest applicable
        # correction becomes the final value.
        ...
        seq_column = SEQUENCE_COLUMNS.get(
            domain
        )
        ...
        for _, correction in corrections.iterrows():
            ...
            if seq_column is None:
                ...
                if seq_column not in result.columns:
                    continue
    ...
    def _reference_for_lab(
        ...
        return {
            ...
            if "LBORRESU" in refs.columns
            else unit,
            "lab": clean_string(
                row["LAB"]
            )
            if "LAB" in refs.columns
            else None,
        }
    ...
    def _normalize_lab_row(self, row):
        ...
        test_code = clean_string(
            row.get("LBTESTCD")
        )

        original_unit = clean_string(
            row.get("LBORRESU")
        )
        ...
        result["LB_VALUE_STATUS"] = (
            status
        )

        result["LB_NORMALIZED_UNIT"] = (
            normalized_unit
        )
    ...
    def build(self, cut=None):
        ...
        self.protocol_version = (
            self._protocol_for_cut(
                cut
            )
        )

        # Reset indexes and graph.
        self.data = {}
        self.nodes = {}
        self.edges = []
        self.subject_index = {}
        ...
        for domain, df in self.data.items():
            ...
            for row_position, row in (
                ...
                # Connect record to subject.
                if (
                    usubjid
                    in self.subject_index
                ):
